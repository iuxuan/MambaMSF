...
def load_data(data_set_name, data_path='./data'):
    ...
    elif data_set_name == 'HyRank':
        data = sio.loadmat(os.path.join(data_path, 'HyRank', 'Dioni.mat'))['ori_data']
        labels = sio.loadmat(os.path.join(data_path, 'HyRank', 'Dioni_gt_out68.mat'))['map']
    elif data_set_name == 'HyRank_Loukia':
        data = sio.loadmat(os.path.join(data_path, 'HyRank_Loukia', 'Loukia.mat'))['ori_data']
        labels = sio.loadmat(os.path.join(data_path, 'HyRank_Loukia', 'Loukia_gt_out68.mat'))['map']
    elif data_set_name == 'Pavia_C':
        data = sio.loadmat(os.path.join(data_path, 'Pavia_C', 'pavia.mat'))['pavia']
        labels = sio.loadmat(os.path.join(data_path, 'Pavia_C', 'pavia_gt_7.mat'))['pavia_gt_7']
    elif data_set_name == 'Pavia_U':
        data = sio.loadmat(os.path.join(data_path, 'Pavia_U', 'paviaU.mat'))['paviaU']
        labels = sio.loadmat(os.path.join(data_path, 'Pavia_U', 'paviaU_gt_7.mat'))['paviaU_gt_7']
    elif data_set_name == 'shanghai':
        data = sio.loadmat(os.path.join(data_path, 'shanghai', 'shanghai.mat'))['DataCube1']
        labels = sio.loadmat(os.path.join(data_path, 'shanghai', 'shanghai_gt.mat'))['gt1']
    elif data_set_name == 'hangzhou':
        data = sio.loadmat(os.path.join(data_path, 'hangzhou', 'hangzhou.mat'))['DataCube2']
        labels = sio.loadmat(os.path.join(data_path, 'hangzhou', 'hangzhou_gt.mat'))['gt2']
    elif data_set_name == 'Indiana1':
        data = sio.loadmat(os.path.join(data_path, 'Indiana1', 'Indiana1.mat'))['DataCube1']
        labels = sio.loadmat(os.path.join(data_path, 'Indiana1', 'Indiana1_gt.mat'))['gt1']
    elif data_set_name == 'Indiana2':
        data = sio.loadmat(os.path.join(data_path, 'Indiana2', 'Indiana2.mat'))['DataCube2']
        labels = sio.loadmat(os.path.join(data_path, 'Indiana2', 'Indiana2_gt.mat'))['gt2']   
    elif data_set_name == 'Pavia1':
        data = sio.loadmat(os.path.join(data_path, 'Pavia1', 'Pavia.mat'))['pavia']
        labels = sio.loadmat(os.path.join(data_path, 'Pavia1', 'Pavia_gt.mat'))['pavia_gt'] 
    elif data_set_name == 'Pavia2':
        data = sio.loadmat(os.path.join(data_path, 'Pavia2', 'PaviaU.mat'))['paviaU']
        labels = sio.loadmat(os.path.join(data_path, 'Pavia2', 'PaviaU_gt.mat'))['paviaU_gt'] 
    elif data_set_name == 'shanghai_1':
        data = sio.loadmat(os.path.join(data_path, 'shanghai_1', 'Shanghai-Hangzhou.mat'))['DataCube1']
        labels = sio.loadmat(os.path.join(data_path, 'shanghai_1', 'Shanghai-Hangzhou.mat'))['gt1'] 
    elif data_set_name == 'hangzhou_1':
        data = sio.loadmat(os.path.join(data_path, 'hangzhou_1', 'Shanghai-Hangzhou.mat'))['DataCube2']
        labels = sio.loadmat(os.path.join(data_path, 'hangzhou_1', 'Shanghai-Hangzhou.mat'))['gt2'] 
    elif data_set_name == 'Houston13':    
        data_file_path = os.path.join(data_path, 'Houston13', 'Houston13.mat')
        labels_file_path = os.path.join(data_path, 'Houston13', 'Houston13_7gt.mat')
        # 加载数据
        with h5py.File(data_file_path, 'r') as f:
            data = np.array(f['ori_data'])
        # 加载标签
        with h5py.File(labels_file_path, 'r') as f:
            labels = np.array(f['map'])
        # 如果数据是以MATLAB的格式存储的，可能需要进行转置
        data = np.transpose(data, (2, 1, 0))
        labels = np.transpose(labels)
    
    elif data_set_name == 'Houston18':
        data_file_path = os.path.join(data_path, 'Houston18', 'Houston18.mat')
        labels_file_path = os.path.join(data_path, 'Houston18', 'Houston18_7gt.mat')
        # 加载数据
        with h5py.File(data_file_path, 'r') as f:
            data = np.array(f['ori_data'])
        # 加载标签
