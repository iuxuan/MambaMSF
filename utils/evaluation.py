...
class Evaluator(object):
    ...
    def _generate_matrix(self, gt_image, pre_image):
        ...
        # Ensure count has the correct size before reshaping
        if count.shape[0] > self.num_class ** 2:
             count = count[:self.num_class ** 2]
        elif count.shape[0] < self.num_class ** 2:
             # Pad with zeros if count is smaller than expected
             pad_size = self.num_class ** 2 - count.shape[0]
             count = np.pad(count, (0, pad_size), 'constant')
